...
eststo r2mp: estpost  ttest $characteristics1, by(patent_lib) unequal
esttab r2mp using "results\TabB1a.tex", cells("mu_2(fmt(%9.2f) label(Patent Libraries)) mu_1(fmt(%9.2f) label(Control Libraries)) b(fmt(%9.2f) label(Diff)) p(fmt(%9.2f) label(P-Value))") fragment booktabs nodepvar noobs nomtitle nonumbers label star(* 0.10 ** 0.05 *** 0.01) staraux  replace

eststo r2mp: estpost  ttest $characteristics3, by(patent_lib) unequal
esttab r2mp using "results\TabB1b.tex", cells("mu_2(fmt(%9.2f) label(Patent Libraries)) mu_1(fmt(%9.2f) label(Control Libraries)) b(fmt(%9.2f) label(Diff)) p(fmt(%9.2f) label(P-Value))") fragment booktabs nodepvar noobs nomtitle nonumbers label star(* 0.10 ** 0.05 *** 0.01) staraux  replace