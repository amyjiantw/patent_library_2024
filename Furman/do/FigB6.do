...
keep if patent_attorneys_15m!=.
xtset identifier appln_y
replace pat_pop_distance= pat_attorneys_distance
...
twoway (rcap upper lower year, lpattern(solid) color(edkblue)) (connected diff year , lpattern(dash) color(edkblue))  , ytitle(Excess Patents p.c. within 15 miles) legend(off) xline(0, lcolor(gs10))    xtitle("Year relative to opening") yline(0) ysize(5) yscale(range(-2 10)) ylabel(-2(2)10) xlabel(-5(1)5) ylabel(, labsize(large)) xlabel(, labsize(large))
graph export "results\FigB6.pdf", replace
