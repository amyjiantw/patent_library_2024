...
replace pat_pop_distance=pat_mainarea30_3
...
twoway (rcap upper lower year, lpattern(solid) color(edkblue)) (connected diff year , lpattern(dash) color(edkblue))  , ytitle(Excess Patents per 100'000 persons) legend(off) xline(0, lcolor(gs10))    xtitle("Year relative to opening") yline(0) ysize(5) yscale(range(-2 4)) ylabel(-2(1)4) xlabel(-5(1)5) ylabel(, labsize(large)) xlabel(, labsize(large))
graph export "results\FigC2.pdf", replace
